...
class RunPodClient:
 ...
 def separate_audio(self,file_path,job_id,output_format='mp3',quality='high'):
  try:
   ...
   run_url=f"https://api.runpod.ai/v2/{self.endpoint_id}/run";response=requests.post(run_url,json=payload,headers=headers,timeout=120)
   if response.status_code!=200:return {"error": f"RunPod API error {response.status_code}: {response.text[:500] if response.text else 'No error message'}"}
  ...
  except Exception as e:return {"error": f"RunPod connection error: {str(e)}"}
 def _poll_job_status(self,runpod_job_id):
  headers={"Authorization":f"Bearer {self.api_key}","Content-Type":"application/json"};status_url=f"https://api.runpod.ai/v2/{self.endpoint_id}/status/{runpod_job_id}";max_attempts=60
